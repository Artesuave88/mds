<script>
  ...
  const ACTIVE_LINK_CLASS = "bg-brand-text text-brand-surface shadow-[0_10px_25px_rgba(16,17,20,0.16)]";
...
<header class="sticky top-0 z-50 border-b border-brand-border/80 bg-brand-surface/92 backdrop-blur-xl">
  <div class="mx-auto flex h-20 w-full max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
    <a href="/" class="inline-flex items-center gap-3 rounded-lg px-2 py-1 text-brand-text">
      <img
        src="/brand/logo-mark.png"
        alt=""
        aria-hidden="true"
        class="h-12 w-12 rounded-lg border border-brand-primary/50 bg-black object-contain p-1 brightness-110 contrast-125 saturate-150 shadow-[0_12px_26px_rgba(16,17,20,0.15)]"
      />
      <div class="hidden sm:block">
        <span class="block text-sm font-bold tracking-wide">Midas Web Development</span>
        <span class="font-['Space_Mono'] text-[10px] uppercase tracking-[0.14em] text-brand-text/50">Custom sites for UK businesses</span>
      </div>
    </a>
    ...
    <nav class="hidden items-center gap-1 md:flex">
      {#each navItems as item}
        <a
          class={`rounded-lg px-3 py-2 text-sm font-semibold transition ${item.href === "/contact" ? CTA_CLASS : activeHref === item.href ? ACTIVE_LINK_CLASS : INACTIVE_LINK_CLASS}`}
          href={item.href}
    ...
    <button
      ...
      class="inline-flex h-11 w-11 items-center justify-center rounded-lg border border-brand-border/80 bg-brand-surface/70 text-brand-text/85 md:hidden"
      on:click={() => {
